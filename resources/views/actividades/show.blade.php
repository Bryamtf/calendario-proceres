...
@section('contenido')
    <div x-data="{ modalAprobar: false, modalRechazar: false, modalCancelar: false, participacionAbierta: {} }"
        class="max-w-2xl mx-auto p-5 md:p-8 space-y-6">

        <a href="{{ route('actividades.index') }}"
            class="inline-flex items-center gap-1.5 text-sm text-ink/50 hover:text-ink transition-colors -mt-2">
                ...
                @can('update', $actividad)
                    ...
                    @if(!$actividad->estadoActualEnum()->esTerminal() || $esRechazada)
                        <a href="{{ route('actividades.edit', $actividad) }}"
                            class="text-ink/30 hover:text-primary transition-colors p-1.5"
                            title="{{ $esRechazada ? 'Corregir y reenviar' : 'Editar' }}">
        ...
        @if($actividad->historialEstados->isNotEmpty())
            <div x-data="{ historialAbierto: false }" class="bg-white border border-line rounded-xl p-5">
                <button @click="historialAbierto = !historialAbierto"
                    class="w-full flex items-center justify-between text-left">
                    <h3 class="text-xs font-medium text-ink/50 uppercase tracking-wide">Historial</h3>
                    <svg class="w-4 h-4 text-ink/40 transition-transform" :class="historialAbierto && 'rotate-180'"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M6 9l6 6 6-6" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </button>

                <div x-show="historialAbierto" x-cloak x-transition class="relative pl-5 border-l-2 border-line space-y-5 mt-5">
                    @foreach($actividad->historialEstados as $registro)
                        <div class="relative">
                            <span class="absolute -left-[25px] top-0.5 w-2.5 h-2.5 rounded-full ring-4 ring-white"
                                style="background: {{ $registro->estadoNuevo->color }}"></span>
                            <p class="text-sm text-ink/80">
                                <span class="font-medium">{{ $registro->usuario->name }}</span>
                                @if($registro->estadoAnterior)
                                    cambió de <span class="font-medium">{{ $registro->estadoAnterior->nombre }}</span> a <span
                                        class="font-medium">{{ $registro->estadoNuevo->nombre }}</span>
                                @else
                                    creó la propuesta <span class="font-medium">({{ $registro->estadoNuevo->nombre }})</span>
                                @endif
                            </p>
                            <p class="text-xs text-ink/40 font-mono mt-0.5">{{ $registro->fecha_cambio->format('d M Y, g:i a') }}
                            </p>
                            @if($registro->comentario)
                                <p class="text-xs text-ink/50 italic mt-1.5 bg-paper rounded px-2.5 py-1.5 inline-block">
                                    "{{ $registro->comentario }}"</p>
                            @endif
                        </div>
                    @endforeach
                </div>
        ...
        {{-- Acciones sobre actividad Aprobada: marcar Realizada o Cancelar (causa mayor) --}}
        @if($actividad->estadoActual->nombre === 'Aprobada')
            @can('marcarRealizada', $actividad)
                <div class="sticky bottom-0 bg-paper pt-2 pb-1 border-t border-line flex gap-3">
                    @can('cancelar', $actividad)
                        <button @click="modalCancelar = true"
                            class="flex-1 border border-brick/30 text-brick text-sm font-medium py-2.5 rounded-lg hover:bg-brick/5 transition-colors">Cancelar</button>
                    @endcan
                    <form method="POST" action="{{ route('actividades.realizar', $actividad) }}" class="flex-1">
                        @csrf
                        <button
                            class="w-full bg-sage text-white text-sm font-medium py-2.5 rounded-lg hover:bg-sage/90 transition-colors">Marcar
        ...
        <div x-show="modalCancelar" x-cloak class="fixed inset-0 bg-black/40 flex items-center justify-center p-5 z-50">
            <div class="bg-white rounded-xl max-w-sm w-full p-6" @click.outside="modalCancelar = false">
                ...
                <form method="POST" action="{{ route('actividades.cancelar', $actividad) }}">
                    @csrf
                    <p class="text-sm text-ink/60 mb-3">Esta actividad ya estaba Aprobada. Indica el motivo de la
                        cancelación (causa mayor) — quedará visible en el historial.</p>
                    <textarea name="motivo" class="form-input mb-5" rows="3" required></textarea>
                    <div class="flex gap-3">
                        <button type="button" @click="modalCancelar = false"
                            class="flex-1 border border-line text-sm font-medium py-2.5 rounded-lg hover:bg-paper transition-colors">Volver</button>
                        <button
                            class="flex-1 bg-brick text-white text-sm font-medium py-2.5 rounded-lg hover:bg-brick/90 transition-colors">Confirmar
                            cancelación</button>
                    </div>
