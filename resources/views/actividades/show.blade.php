...
@section('contenido')
<div x-data="{ modalAprobar: false, modalRechazar: false, modalCancelar: false, participacionAbierta: {} }" class="max-w-2xl mx-auto p-5 md:p-8 space-y-6">

    <a href="{{ route('actividades.index') }}" class="inline-flex items-center gap-1.5 text-sm text-ink/50 hover:text-ink transition-colors -mt-2">
    ...
    <div class="bg-white border border-line rounded-xl p-5 space-y-4">
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div><p class="text-[11px] text-ink/45 mb-0.5">Fecha</p><p class="text-sm font-mono">{{ $actividad->fecha->format('d M Y') }}</p></div>
            <div><p class="text-[11px] text-ink/45 mb-0.5">Hora</p><p class="text-sm font-mono">{{ \Carbon\Carbon::parse($actividad->hora_inicio)->format('g:i a') }} – {{ \Carbon\Carbon::parse($actividad->hora_fin)->format('g:i a') }}</p></div>
            <div><p class="text-[11px] text-ink/45 mb-0.5">Lugar</p><p class="text-sm">{{ $actividad->lugar }}</p></div>
        </div>
    ...
    <div class="bg-white border border-line rounded-xl p-5">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div><p class="font-display font-semibold text-xl">{{ $actividad->asistencia_esperada ?? '—' }}</p><p class="text-[11px] text-ink/45">Asistencia</p></div>
            ...
            @foreach(['miembro_nuevo' => ['label' => 'Miembros nuevos', 'valor' => $actividad->miembros_nuevos], 'amigo_ensenanza' => ['label' => 'Amigos', 'valor' => $actividad->amigos_ensenanza], 'menos_activo' => ['label' => 'Menos activos', 'valor' => $actividad->miembros_menos_activos]] as $tipo => $info)
                ...
                <div>
                    ...
                    @if($nombres->isNotEmpty())
                        <button @click="participacionAbierta['{{ $tipo }}'] = !participacionAbierta['{{ $tipo }}']" class="text-[11px] text-accent hover:text-accent/80 flex items-center gap-0.5 transition-colors">
                            <svg class="w-3 h-3 transition-transform" :class="participacionAbierta['{{ $tipo }}'] && 'rotate-90'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 6l6 6-6 6" stroke-linecap="round"/></svg>
                            Ver nombres
                        </button>
    ...
    @if($actividad->solicita_presupuesto)
        <div class="bg-white border border-line rounded-xl p-5">
            <div class="flex items-center justify-between mb-3.5">
                ...
                <span class="font-mono text-lg font-semibold text-accent">${{ number_format($actividad->montoTotalSolicitado(), 2) }}</span>
            </div>
            <div class="space-y-2">
        ...
        <div class="bg-white border border-line rounded-xl p-5">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($actividad->recursos as $recurso)
                    <span class="text-xs bg-paper border border-line px-3 py-1.5 rounded-full">
                        {{ $recurso->nombre }}{{ $recurso->pivot->detalle ? ': '.$recurso->pivot->detalle : '' }}
                    </span>
                @endforeach
            </div>
        </div>
    ...
    @can('comentar', $actividad)
        <div class="bg-white border border-line rounded-xl p-5">
            ...
            <div class="space-y-3 mb-4">
                @forelse($actividad->comentarios as $comentario)
                    <div class="flex gap-3">
                        <div class="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center font-display font-semibold text-primary text-[11px] shrink-0">
                        ...
                        <div class="bg-paper rounded-lg px-3.5 py-2.5 flex-1">
                            <div class="flex items-center justify-between mb-0.5">
                                ...
                                <span class="text-[10px] text-ink/35 font-mono">{{ $comentario->created_at->format('d M') }}</span>
                            </div>
            ...
            <form method="POST" action="{{ route('actividades.comentar', $actividad) }}" class="flex gap-2">
                @csrf
                <input type="text" name="comentario" placeholder="Agregar un comentario..." class="form-input text-sm" required>
                <button class="bg-primary text-white text-xs font-medium px-4 rounded-lg hover:bg-primary-light transition-colors shrink-0">Comentar</button>
            </form>
        </div>
    @endcan
    ...
    @if($actividad->estadoActual->nombre === 'Pendiente')
        @can('aprobar', $actividad)
            <div class="sticky bottom-0 bg-paper pt-2 pb-1 border-t border-line flex gap-3">
                <button @click="modalRechazar = true" class="flex-1 border border-brick/30 text-brick text-sm font-medium py-2.5 rounded-lg hover:bg-brick/5 transition-colors">Rechazar</button>
                <button @click="modalAprobar = true" class="flex-1 bg-sage text-white text-sm font-medium py-2.5 rounded-lg hover:bg-sage/90 transition-colors">Aprobar</button>
            </div>
        @endcan
    @endif

    {{-- Cancelar (actividad Aprobada, causa mayor) --}}
    @if($actividad->estadoActual->nombre === 'Aprobada')
        @can('cancelar', $actividad)
            <div class="sticky bottom-0 bg-paper pt-2 pb-1 border-t border-line">
                <button @click="modalCancelar = true" class="w-full border border-brick/30 text-brick text-sm font-medium py-2.5 rounded-lg hover:bg-brick/5 transition-colors">Cancelar actividad</button>
            </div>
        @endcan
    @endif
    ...
    @if($actividad->estadoActual->nombre === 'No Procesada')
        @can('migrarANuevoTrimestre', $actividad)
            <form method="POST" action="{{ route('actividades.migrar', $actividad) }}">
                @csrf
                <button class="w-full bg-primary text-white text-sm font-medium py-2.5 rounded-lg hover:bg-primary-light transition-colors">Migrar al trimestre activo</button>
            </form>
        @endcan
    @endif
    ...
    <div x-show="modalAprobar" x-cloak class="fixed inset-0 bg-black/40 flex items-center justify-center p-5 z-50">
        <div class="bg-white rounded-xl max-w-sm w-full p-6" @click.outside="modalAprobar = false">
            ...
            <p class="text-sm text-ink/60 mb-5 leading-relaxed">Confirmas que el Consejo de Obispado acordó aprobar esta actividad. Quedará registrada bajo tu usuario como quien ejecutó la decisión.</p>
            <div class="flex gap-3">
                <button @click="modalAprobar = false" class="flex-1 border border-line text-sm font-medium py-2.5 rounded-lg hover:bg-paper transition-colors">Cancelar</button>
                <form method="POST" action="{{ route('actividades.aprobar', $actividad) }}" class="flex-1">
                    @csrf
                    <button class="w-full bg-sage text-white text-sm font-medium py-2.5 rounded-lg hover:bg-sage/90 transition-colors">Confirmar</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div x-show="modalRechazar" x-cloak class="fixed inset-0 bg-black/40 flex items-center justify-center p-5 z-50">
        <div class="bg-white rounded-xl max-w-sm w-full p-6" @click.outside="modalRechazar = false">
            ...
            <form method="POST" action="{{ route('actividades.rechazar', $actividad) }}">
                ...
                <div class="flex gap-3">
                    <button type="button" @click="modalRechazar = false" class="flex-1 border border-line text-sm font-medium py-2.5 rounded-lg hover:bg-paper transition-colors">Cancelar</button>
                    <button class="flex-1 bg-brick text-white text-sm font-medium py-2.5 rounded-lg hover:bg-brick/90 transition-colors">Confirmar rechazo</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="modalCancelar" x-cloak class="fixed inset-0 bg-black/40 flex items-center justify-center p-5 z-50">
        <div class="bg-white rounded-xl max-w-sm w-full p-6" @click.outside="modalCancelar = false">
            <h3 class="font-display font-semibold text-lg mb-1.5">Cancelar actividad</h3>
            <form method="POST" action="{{ route('actividades.cancelar', $actividad) }}">
                @csrf
                <p class="text-sm text-ink/60 mb-3">Esta actividad ya estaba Aprobada. Indica el motivo de la cancelación (causa mayor) — quedará visible en el historial.</p>
                <textarea name="motivo" class="form-input mb-5" rows="3" required></textarea>
                <div class="flex gap-3">
                    <button type="button" @click="modalCancelar = false" class="flex-1 border border-line text-sm font-medium py-2.5 rounded-lg hover:bg-paper transition-colors">Volver</button>
                    <button class="flex-1 bg-brick text-white text-sm font-medium py-2.5 rounded-lg hover:bg-brick/90 transition-colors">Confirmar cancelación</button>
                </div>
            </form>
        </div>
    </div>
