...
@section('contenido')
    <div x-data="actividadForm()" x-init="init()" class="max-w-3xl mx-auto p-5 md:p-10">
        ...
        <a href="{{ route('actividades.show', $actividad) }}"
            class="inline-flex items-center gap-1.5 text-sm text-ink/50 hover:text-ink transition-colors mb-6">
            ...
            <template x-for="(label, i) in pasos" :key="i">
                <div class="flex items-center gap-2 flex-1">
                    <div class="h-2 rounded-full transition-all"
                        :class="paso === i ? 'w-[22px] bg-accent' : (paso > i ? 'w-2 bg-sage' : 'w-2 bg-line')"></div>
                    <span class="text-[11px] font-medium hidden sm:block" :class="paso === i ? 'text-ink' : 'text-ink/35'"
                        x-text="label"></span>
        ...
        <form x-ref="form" method="POST" action="{{ route('actividades.update', $actividad) }}" class="space-y-6">
            ...
            <div x-show="paso === 0" x-cloak class="bg-white border border-line rounded-xl p-6 md:p-7 space-y-5">
                <div>
                    ...
                    <p class="text-sm text-ink/50">Cambiar la fecha, el presupuesto o la organización regresa la actividad a
                        Pendiente.</p>
                </div>

                <div>
                    <label class="form-label">Nombre de la actividad</label>
                    <input type="text" name="nombre" class="form-input" value="{{ old('nombre', $actividad->nombre) }}"
                        required>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="lugar" class="form-input" value="{{ old('lugar', $actividad->lugar) }}"
                        required>
                </div>

                <div>
                    <label class="form-label">Objetivo</label>
                    <textarea name="objetivo" class="form-input" rows="2"
                ...
                <div>
                    ...
                    <textarea name="descripcion" class="form-input"
                        rows="3">{{ old('descripcion', $actividad->descripcion) }}</textarea>
                </div>
            </div>
            ...
            <div x-show="paso === 1" x-cloak class="bg-white border border-line rounded-xl p-6 md:p-7 space-y-5">
                ...
                <div>
                    ...
                    <input type="number" name="asistencia_esperada" min="0" class="form-input font-mono w-full sm:w-40"
                        value="{{ old('asistencia_esperada', $actividad->asistencia_esperada) }}">
                ...
                <div class="border-t border-line pt-5 space-y-5">
                    <template x-for="grupo in gruposParticipacion" :key="grupo.tipo">
                        <div>
                            ...
                            <input type="number" min="0" class="form-input font-mono w-full sm:w-40"
                                :name="grupo.campoConteo" :value="grupo.conteoInicial">

                            <button type="button" @click="grupo.abierto = !grupo.abierto"
                                class="mt-2 text-xs font-medium text-accent hover:text-accent/80 flex items-center gap-1 transition-colors">
                                <svg class="w-3.5 h-3.5 transition-transform" :class="grupo.abierto && 'rotate-90'"
                                    viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M9 6l6 6-6 6" stroke-linecap="round" />
                                </svg>
                                <span
                                    x-text="grupo.abierto ? 'Ocultar nombres' : (grupo.nombres.length ? grupo.nombres.length + ' nombre(s) agregado(s)' : '+ Agregar nombres (opcional)')"></span>
                            </button>
                            ...
                            <div x-show="grupo.abierto" x-cloak class="mt-3 p-3.5 bg-paper border border-line rounded-lg">
                                <div class="flex flex-wrap gap-2 mb-2.5" x-show="grupo.nombres.length">
                                    <template x-for="(nombre, i) in grupo.nombres" :key="i">
                                        <span
                                            class="inline-flex items-center gap-1.5 text-xs bg-white border border-line pl-2.5 pr-1.5 py-1 rounded-full">
                                            <span x-text="nombre"></span>
                                            <input type="hidden" :name="'participantes[' + grupo.tipo + '_' + i + '][tipo]'"
                                                :value="grupo.tipo">
                                            <input type="hidden"
                                                :name="'participantes[' + grupo.tipo + '_' + i + '][nombre]'"
                                                :value="nombre">
                                            <button type="button" @click="grupo.nombres.splice(i, 1)"
                                                class="text-ink/30 hover:text-brick transition-colors">
                                ...
                                <div class="flex gap-2">
                                    <input type="text" class="form-input text-sm bg-white" x-model="grupo.borrador"
                                        @keydown.enter.prevent="agregarParticipante(grupo)">
                                    <button type="button" @click="agregarParticipante(grupo)"
                                        class="shrink-0 bg-primary text-white text-xs font-medium px-3.5 rounded-lg hover:bg-primary-light transition-colors">Agregar</button>
                                </div>
                            </div>
                        </div>
                    </template>
            ...
            <div x-show="paso === 2" x-cloak class="bg-white border border-line rounded-xl p-6 md:p-7 space-y-5">
                ...
                <label
                    class="flex items-center gap-3 p-3.5 border border-line rounded-lg cursor-pointer hover:bg-paper/60 transition-colors">
                    <input type="checkbox" name="solicita_presupuesto" value="1" x-model="solicitaPresupuesto"
                ...
                <div x-show="solicitaPresupuesto" x-cloak class="space-y-5 pt-1">
                    <div class="flex items-center gap-1 bg-paper border border-line rounded-lg p-1 w-fit">
                        <button type="button" @click="modoPresupuesto = 'aproximado'"
                            :class="modoPresupuesto === 'aproximado' ? 'bg-primary text-white' : 'text-ink/50 hover:text-ink'"
                            class="px-3.5 py-1.5 rounded-md text-xs font-medium transition-colors">Monto aproximado</button>
                        <button type="button" @click="modoPresupuesto = 'desglose'"
                            :class="modoPresupuesto === 'desglose' ? 'bg-primary text-white' : 'text-ink/50 hover:text-ink'"
                            class="px-3.5 py-1.5 rounded-md text-xs font-medium transition-colors">Desglose por
                            rubro</button>
                    </div>

                    {{-- MODO APROXIMADO --}}
                    <div x-show="modoPresupuesto === 'aproximado'" x-cloak class="space-y-4">
                        <input type="hidden" name="presupuesto_items[aprox][categoria_presupuesto_id]" value=""
                            :disabled="!(solicitaPresupuesto && modoPresupuesto === 'aproximado')">
                        <div>
                            <label class="form-label">Monto aproximado</label>
                            <div class="relative">
                                <span
                                    class="absolute left-3 top-1/2 -translate-y-1/2 text-ink/40 font-mono text-sm">$</span>
                                <input type="number" min="0" step="0.01" name="presupuesto_items[aprox][monto]"
                                    class="form-input font-mono pl-6"
                        ...
                        <div>
                            ...
                            <textarea name="presupuesto_items[aprox][justificacion]" class="form-input" rows="2"
                                :disabled="!(solicitaPresupuesto && modoPresupuesto === 'aproximado')">{{ old('presupuesto_items.aprox.justificacion', $esAproximado && $itemsExistentes->isNotEmpty() ? $itemsExistentes->first()->justificacion : '') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div x-show="modoPresupuesto === 'desglose'" x-cloak class="space-y-3">
                        <template x-for="(item, idx) in rubros" :key="idx">
                            <div
                                class="flex flex-col sm:flex-row gap-2.5 items-start sm:items-center p-3 border border-line rounded-lg bg-paper/40">
                                <select x-model="item.categoria_presupuesto_id"
                                    :name="'presupuesto_items[' + idx + '][categoria_presupuesto_id]'"
                                    class="form-input sm:w-40 shrink-0"
                                    :disabled="!(solicitaPresupuesto && modoPresupuesto === 'desglose')">
                                ...
                                <div class="relative sm:w-32 shrink-0 w-full">
                                    <span
                                        class="absolute left-3 top-1/2 -translate-y-1/2 text-ink/40 font-mono text-sm">$</span>
                                    <input type="number" min="0" step="0.01" :name="'presupuesto_items[' + idx + '][monto]'"
                                        class="form-input font-mono pl-6" placeholder="0.00" x-model.number="item.monto"
                                        :disabled="!(solicitaPresupuesto && modoPresupuesto === 'desglose')">
                                </div>
                                <input type="text" :name="'presupuesto_items[' + idx + '][justificacion]'"
                                    class="form-input flex-1" placeholder="Nota (opcional)" x-model="item.justificacion"
                                    :disabled="!(solicitaPresupuesto && modoPresupuesto === 'desglose')">
                                <button type="button" @click="rubros.splice(idx, 1)"
                                    class="text-ink/30 hover:text-brick transition-colors p-1.5 shrink-0">
                        ...
                        <button type="button"
                            @click="rubros.push({ categoria_presupuesto_id: '', monto: 0, justificacion: '' })"
                            class="text-sm font-medium text-accent hover:text-accent/80 flex items-center gap-1.5 transition-colors">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div x-show="paso === 3" x-cloak class="bg-white border border-line rounded-xl p-6 md:p-7 space-y-5">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    @foreach($recursos as $recurso)
                        <label
                            class="flex items-center gap-2.5 p-3 border border-line rounded-lg cursor-pointer hover:bg-paper/60 transition-colors">
                            <input type="checkbox" name="recursos[]" value="{{ $recurso->id }}"
                                class="w-4 h-4 rounded accent-accent" @if($recurso->nombre === 'Otro') x-model="otroSeleccionado"
                                @endif @checked(in_array($recurso->id, $recursosSeleccionados))>
                ...
                @if($recursoOtro)
                    ...
                    <div x-show="otroSeleccionado" x-cloak>
                        ...
                        <input type="text" name="recursos_detalle[{{ $recursoOtro->id }}]" class="form-input"
                            placeholder="¿Qué recurso adicional necesitas?"
                            value="{{ old('recursos_detalle.' . $recursoOtro->id, $detalleOtro) }}">
                    </div>
                @endif
            </div>
            ...
            <div x-show="paso === 4" x-cloak class="bg-white border border-line rounded-xl p-6 md:p-7 space-y-5">
                ...
                <div class="divide-y divide-line border border-line rounded-lg overflow-hidden">
                    <div class="px-4 py-3 flex justify-between gap-4"><span
                            class="text-xs text-ink/50 shrink-0">Actividad</span><span
                            class="text-sm font-medium text-right" x-text="resumen.nombre || '—'"></span></div>
                    <div class="px-4 py-3 flex justify-between gap-4"><span
                            class="text-xs text-ink/50 shrink-0">Organización</span><span class="text-sm text-right"
                            x-text="resumen.organizacion || '—'"></span></div>
                    <div class="px-4 py-3 flex justify-between gap-4"><span class="text-xs text-ink/50 shrink-0">Fecha y
                            hora</span><span class="text-sm font-mono text-right" x-text="resumen.fecha"></span></div>
                    <div class="px-4 py-3 flex justify-between gap-4"><span
                            class="text-xs text-ink/50 shrink-0">Lugar</span><span class="text-sm text-right"
                            x-text="resumen.lugar || '—'"></span></div>
                    <div class="px-4 py-3 flex justify-between gap-4"><span class="text-xs text-ink/50 shrink-0">Asistencia
                            esperada</span><span class="text-sm font-mono text-right"
                            x-text="resumen.asistencia || '0'"></span></div>
                    <div class="px-4 py-3 flex justify-between gap-4"><span class="text-xs text-ink/50 shrink-0">Miembros
                            nuevos</span><span class="text-sm text-right" x-text="resumen.miembrosNuevos"></span></div>
                    <div class="px-4 py-3 flex justify-between gap-4"><span class="text-xs text-ink/50 shrink-0">Amigos en
                            enseñanza</span><span class="text-sm text-right" x-text="resumen.amigos"></span></div>
                    <div class="px-4 py-3 flex justify-between gap-4"><span class="text-xs text-ink/50 shrink-0">Menos
                            activos</span><span class="text-sm text-right" x-text="resumen.menosActivos"></span></div>
                    ...
                    <div class="px-4 py-3 flex justify-between gap-4"><span
                            class="text-xs text-ink/50 shrink-0">Recursos</span><span class="text-sm text-right"
                            x-text="resumen.recursos"></span></div>
                </div>

                <div class="bg-accent/5 border border-accent/20 rounded-lg p-4 flex gap-3">
                    <svg class="w-5 h-5 text-accent shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="1.8">
                        <path
                    ...
                    <p class="text-sm text-ink/70">
                        @if($actividad->estadoActual->nombre === 'Rechazada')
                            Esta actividad fue <span class="font-medium text-ink">rechazada</span>. Al guardar, se reenvía
                            automáticamente como <span class="font-medium text-ink">Pendiente</span> para que el Consejo la
                            revise de nuevo.
                        @elseif($actividad->estadoActual->nombre === 'Aprobada')
                            Si cambiaste <span class="font-medium text-ink">fecha, presupuesto u organización</span>, esta
                            actividad volverá a estado <span class="font-medium text-ink">Pendiente</span> para una nueva
                            revisión del Consejo. Cambios de lugar, hora o descripción no la afectan.
                        @else
                            Los cambios se guardan directamente — esta actividad sigue en estado <span
                                class="font-medium text-ink">{{ $actividad->estadoActual->nombre }}</span>.
                        @endif
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between pt-2">
                <button type="button" @click="paso > 0 && paso--" x-show="paso > 0"
                    class="text-sm font-medium text-ink/60 hover:text-ink px-4 py-2.5 transition-colors">← Atrás</button>
                <div class="flex-1"></div>
                <button type="button" @click="avanzar()" x-show="paso < 4"
                    class="bg-primary text-white text-sm font-medium px-5 py-2.5 rounded-lg hover:bg-primary-light transition-colors">Continuar
                    →</button>
                <button type="submit" x-show="paso === 4"
                    class="bg-accent text-white text-sm font-medium px-5 py-2.5 rounded-lg hover:bg-accent/90 transition-colors">{{ $actividad->estadoActual->nombre === 'Rechazada' ? 'Corregir y reenviar' : 'Guardar cambios' }}</button>
            </div>
        </form>
    </div>
@endsection
